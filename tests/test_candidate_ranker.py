...
class CandidateRankerTester(unittest.TestCase):
    ...
    def test_candidates_ranked_according_to_similarity_score(self):
        target = self.minhasher.create_document_sketch("file1", {"aa", "ab", "ba"})
        candidate1 = self.minhasher.create_document_sketch("file2", {"aa", "ab", "ba"})
        candidate2 = self.minhasher.create_document_sketch("file3", {"aa", "ab", "bc"})
    ...
    def test_tie_candidates_sorted_according_to_insertion_order(self):
        target = self.minhasher.create_document_sketch("file1", {"aa", "ab", "ba"})
        candidate1 = self.minhasher.create_document_sketch("file2", {"aa", "ab", "bc"})
        candidate1.doc_id = "c_first"
        candidate2 = self.minhasher.create_document_sketch("file3", {"aa", "ab", "bc"})
        candidate2.doc_id = "a_second"
        candidate3 = self.minhasher.create_document_sketch("file4", {"aa", "ab", "bc"})
